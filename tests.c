...
void countColumsTest(char *inFile, int expected){
    FILE *currFile = null;
    currFile = fopen(inFile,"r");
    if(currFile == NULL){
        printf("%s\n","Error: File is not present");
...
void maxFieldTest(int colIndex, char *inFile, double expected){
    ...
    currFile = fopen(inFile,"r");
    if(currFile == NULL){
        printf("%s","Error: File is not present");
        return;
    }
...
void findRecordsTest(int colIndex, char *value, char *inFile, int expectedN, char **expectedStrs, char **actualStrs){
    ...
    int actualN = findRecordsTest(0, colIndex, value, currFile, actualStrs);
...
Test(max, test00) { runTest("test.csv",100.0); }
